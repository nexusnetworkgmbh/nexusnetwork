'use client';
import {googleOAuthEnabled} from '@/lib/features';
...
export function AuthPage({mode,notice}:{mode:string;notice?:string}){
 ...
 {(login||register)&&<div className="oauth"><form action={oauth}><button disabled={!ready||!googleOAuthEnabled} className="oauth-button">Mit Google fortfahren{!googleOAuthEnabled?' · demnächst':''}</button></form><div className="separator">oder mit E-Mail</div></div>}
 ...
 {register&&<label className="check"><input type="checkbox" name="consent" required/><span>Ich habe die <a href={'/datenschutz/'}>Datenschutzhinweise</a> gelesen.</span></label>}
 </ActionForm>
