...
const app=process.env.NEXT_PUBLIC_SITE_URL||'https://nexusnetwork.pro';
...
Object.assign(env,{NODE_ENV:'production',NEXT_TELEMETRY_DISABLED:'1',NEXT_PUBLIC_SITE_URL:app.replace(/\/$/,'')});
...
function harden(dir){
 for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
  ...
  const policy=["default-src 'none'","script-src 'self' "+[...new Set(hashes)].join(' '),"style-src 'self' 'unsafe-inline'","img-src 'self' data:","font-src 'self'","connect-src 'self'","base-uri 'none'","object-src 'none'","form-action 'self'","frame-src 'none'","worker-src 'none'","manifest-src 'self'"].join('; ');
...
console.log('Public static website generated in /out. Nothing deployed.');
