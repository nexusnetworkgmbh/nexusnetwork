...
const findings=[];let checked=0;let keywordFiles=0;
const patterns=[
 ...
 ['credential-assignment',/\b(?:CLIENT_SECRET|SMTP_PASSWORD|PRIVATE_KEY|PASSWORD|API_KEY|SECRET)\s*[:=]\s*["'][^"'\r\n]{8,}["']/i],
 ];
const keywords=/CLIENT_SECRET|SMTP_PASSWORD|PRIVATE_KEY|DATABASE_URL|password|api_key|secret/i;
function scan(label,bytes){
 ...
 findings.push({file:label,line:i+1,kind});
 }});
...
console.log(JSON.stringify({mode,textFilesScanned:checked,filesContainingCredentialKeywords:keywordFiles,potentialSecrets:findings.length,findings},null,2));
