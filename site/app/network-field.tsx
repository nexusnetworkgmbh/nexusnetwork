...
export function NetworkField(){
  ...
  useEffect(()=>{
    ...
    let frame=0,width=0,height=0,visible=true,pointerX=.62,pointerY=.52,targetX=.62,targetY=.52,touchActive=false,touchBoost=1;
    ...
    const touchMove=(event:TouchEvent)=>{if(motion.matches)return;const touch=event.touches[0];if(touch){touchActive=true;setTarget(touch.clientX,touch.clientY)}};
    const touchEnd=()=>{touchActive=false;targetX=.62;targetY=.52};
    const draw=(time:number)=>{
      if(!visible){frame=0;return}const responsiveness=touchActive?.18:.055;touchBoost+=((touchActive?4.2:1)-touchBoost)*(touchActive?.18:.08);pointerX+=(targetX-pointerX)*responsiveness;pointerY+=(targetY-pointerY)*responsiveness;context.clearRect(0,0,width,height);
      const points=nodes.map(node=>({x:(node.x+(pointerX-.5)*.022*touchBoost*Math.sin(node.phase))*width,y:(node.y+(pointerY-.5)*.025*touchBoost*Math.cos(node.phase))*height,r:node.r,phase:node.phase}));
    ...
    const preference=()=>{if(frame)cancelAnimationFrame(frame);frame=0;pointerX=targetX=.62;pointerY=targetY=.52;touchActive=false;touchBoost=1;if(visible)draw(0)};
