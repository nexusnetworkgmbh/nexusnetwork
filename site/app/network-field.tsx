...
export function NetworkField(){
  ...
  useEffect(()=>{
    ...
    const move=(event:PointerEvent)=>{if(motion.matches)return;const box=canvas.getBoundingClientRect();targetX=(event.clientX-box.left)/box.width;targetY=(event.clientY-box.top)/box.height};
    ...
    resize();observer.observe(canvas);addEventListener('resize',resize,{passive:true});canvas.addEventListener('pointermove',move,{passive:true});document.addEventListener('visibilitychange',visibility);resume();
    return()=>{if(frame)cancelAnimationFrame(frame);motion.removeEventListener('change',preference);observer.disconnect();removeEventListener('resize',resize);canvas.removeEventListener('pointermove',move);document.removeEventListener('visibilitychange',visibility)};
  },[]);
